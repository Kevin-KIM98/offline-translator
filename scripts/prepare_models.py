...
def cmd_manifest(a: argparse.Namespace) -> None:
    ...
    if a.upload_script:
        ...
        lines = [
            "#!/usr/bin/env bash",
            "# Generated by prepare_models.py — uploads model assets to a GitHub release under flat names.",
            "# Idempotent: assets already present with the right size are skipped; uploads are retried",
            "# (gh sometimes reports HTTP 404 from uploads.github.com even though the asset landed).",
            "set -uo pipefail",
            f"TAG=\"${{1:-{a.release_tag or 'models'}}}\"",
            f"FLAT=\"{flat_dir.as_posix()}\"",
            "mkdir -p \"$FLAT\"",
            "gh release view \"$TAG\" >/dev/null 2>&1 || gh release create \"$TAG\" --title \"$TAG\" --notes \"Model assets referenced by assets/manifest.json\"",
            "asset_size() { gh release view \"$TAG\" --json assets -q \".assets[] | select(.name==\\\"$1\\\") | .size\"; }",
            "up() {",
            "  local size have attempt; size=$(stat -c %s \"$1\" 2>/dev/null || stat -f %z \"$1\")",
            "  for attempt in 1 2 3; do",
            "    have=$(asset_size \"$2\")",
            "    if [ \"$have\" = \"$size\" ]; then echo \"ok $2\"; return 0; fi",
            "    [ -n \"$have\" ] && gh release delete-asset \"$TAG\" \"$2\" -y >/dev/null 2>&1",
            "    cp -f \"$1\" \"$FLAT/$2\"",
            "    gh release upload \"$TAG\" \"$FLAT/$2\" >/dev/null 2>&1 || sleep 5",
            "  done",
            "  have=$(asset_size \"$2\")",
            "  if [ \"$have\" = \"$size\" ]; then echo \"ok $2\"; return 0; fi",
            "  echo \"FAILED $2\"; FAILED=$((FAILED+1)); return 1",
            "}",
            "FAILED=0",
        ]
        count = 0
